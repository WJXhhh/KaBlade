...
public class SlashBladeTwoRecipeModdingT extends ShapedOreRecipe {
    ItemStack requiredStateBlade;
    ItemStack requiredStateBlade2;

    public SlashBladeTwoRecipeModdingT(ResourceLocation loc, ItemStack result, ItemStack requiredStateBlade, ItemStack requiredStateBlade2, Object... recipe) {
                ...
                if(!curIs.isEmpty()
                    ...
                    if(!curIs.getTranslationKey().equals(requiredStateBlade.getTranslationKey())){
                        if (curIs.getTranslationKey().equals(requiredStateBlade2.getTranslationKey())){
                            if (!world.isRemote)
                                Main.ModHelper.sendMessageToAll("ThirdResult:" + true);
                            if(getCur(requiredStateBlade2, curIs)){
                                Main.ModHelper.sendMessageToAll("FourthResult:" + true);
                                continue;
                            }
                        }
                        return false;
    ...
    boolean getCur(ItemStack s1,ItemStack s2){
        ...
        for(Map.Entry<Enchantment,Integer> enchant: oldItemEnchants.entrySet())
        {
            ...
            if(level < enchant.getValue()){
                    ...
                    Main.ModHelper.sendMessageToAll("Enflase");
                return false;
            }
        }
        ...
        if(0 < tagValueCompare(ItemSlashBlade.ProudSoul, reqTag, srcTag)){
            Main.ModHelper.sendMessageToAll("Prflase");
            return false;
        }

        if(0 < tagValueCompare(ItemSlashBlade.KillCount, reqTag, srcTag)){
            Main.ModHelper.sendMessageToAll("Kiflase");
            return false;
