...
/**
 * 专为 1.12.2 BufferBuilder (POSITION_COLOR + GL_TRIANGLES) 设计的零内存分配程序化几何体发射器。
 */
public final class VorpalBlackHoleGeometry {
    ...
    public static void ellipsoid(BufferBuilder out,
    ...
    public static void funnel(BufferBuilder out, float inner, float outer, float halfDepth,
    ...
    public static void torus(BufferBuilder out, float cx, float cy, float cz,
        ...
        for (int i = 0; i < majorSegments; i++) {
            ...
            for (int j = 0; j < minorSegments; j++) {
                ...
                quad(out,
                        ...
                        cx + p[3], cy + p[4], cz + p[5],
                        cx + p[6], cy + p[7], cz + p[8],
                        cx + p[9], cy + p[10], cz + p[11],
                        color, alpha);
            }
        }
    }
    ...
    public static void tube(BufferBuilder out,
        ...
        for (int i = 0; i < sides; i++) {
            ...
            quad(out,
                    x0 + ox0, y0 + oy0, z0 + oz0,
                    x1 + ox0, y1 + oy0, z1 + oz0,
                    x1 + ox1, y1 + oy1, z1 + oz1,
                    x0 + ox1, y0 + oy1, z0 + oz1,
                    color, alpha);
        }
    }

    public static void icosahedron(BufferBuilder out, float cx, float cy, float cz,
    ...
    private static void icoPoint(float[] out, int offset, int index,
                                 float sx, float sy, float sz, float rx, float ry, float rz) {
        float[] p = ICOSA_VERTICES[index];
        rotateInto(out, offset, p[0] * sx / PHI, p[1] * sy / PHI, p[2] * sz / PHI, rx, ry, rz);
    }

    public static void tetrahedron(BufferBuilder out, float cx, float cy, float cz, float size,
        ...
        tetraFace(out, q, cx, cy, cz, 0, 2, 1, color, alpha);
        tetraFace(out, q, cx, cy, cz, 0, 1, 3, color, alpha);
        tetraFace(out, q, cx, cy, cz, 0, 3, 2, color, alpha);
        tetraFace(out, q, cx, cy, cz, 1, 2, 3, color, alpha);
    }

    private static void tetraFace(BufferBuilder out, float[] q, float cx, float cy, float cz,
                                  int ia, int ib, int ic, int color, float alpha) {
        int a = ia * 3, b = ib * 3, c = ic * 3;
        tri(out,
                cx + q[a], cy + q[a + 1], cz + q[a + 2],
                cx + q[b], cy + q[b + 1], cz + q[b + 2],
                cx + q[c], cy + q[c + 1], cz + q[c + 2],
                color, alpha);
    }

    public static void wedge(BufferBuilder out, float angle, float inner, float length,
        ...
        float x1 = dx * (inner + length * 0.34F), y1 = dy * (inner + length * 0.34F);
        float x2 = dx * (inner + length * 0.72F), y2 = dy * (inner + length * 0.72F);
        float xt = dx * (inner + length), yt = dy * (inner + length);
        float[] points = SCRATCH.get();
        points[0] = x0 - px * width * 0.10F; points[7] = y0 - py * width * 0.10F;
        points[1] = x0 + px * width * 0.12F; points[8] = y0 + py * width * 0.12F;
        points[2] = x1 + px * width * 0.52F; points[9] = y1 + py * width * 0.52F;
        points[3] = x2 + px * width * (0.22F + jagged * 0.12F); points[10] = y2 + py * width * (0.22F + jagged * 0.12F);
        points[4] = xt; points[11] = yt;
        points[5] = x2 - px * width * (0.28F - jagged * 0.08F); points[12] = y2 - py * width * (0.28F - jagged * 0.08F);
        points[6] = x1 - px * width * 0.58F; points[13] = y1 - py * width * 0.58F;
        float front = depth + thickness, back = depth - thickness;
        for (int i = 1; i < 6; i++) {
            tri(out, points[0], points[7], front, points[i], points[i + 7], front, points[i + 1], points[i + 8], front, color, alpha);
            tri(out, points[0], points[7], back, points[i + 1], points[i + 8], back, points[i], points[i + 7], back, color, alpha);
        }
        for (int i = 0; i < 7; i++) {
            int n = (i + 1) % 7;
            quad(out,
                    points[i], points[i + 7], back,
                    points[n], points[n + 7], back,
                    points[n], points[n + 7], front,
                    points[i], points[i + 7], front,
                    color, alpha);
        }
    }

    public static void glowDisc(BufferBuilder out, float radius, float z, int segments,
                                int centerColor, int edgeColor, float alpha) {
        for (int i = 0; i < segments; i++) {
            float a0 = tau() * i / segments, a1 = tau() * (i + 1) / segments;
            colorTri(out,
                    0, 0, z,
                    (float) Math.cos(a0) * radius, (float) Math.sin(a0) * radius, z,
                    (float) Math.cos(a1) * radius, (float) Math.sin(a1) * radius, z,
                    centerColor, edgeColor, edgeColor, alpha);
        }
    }

    public static void ribbon(BufferBuilder out, float[] points, int pointCount,
        ...
        for (int i = first; i < last; i++) {
            ...
            if (sl < 1.0E-5F) {
                sx = -tz; sy = 0; sz = tx;
                sl = (float) Math.sqrt(sx * sx + sz * sz);
            }
            if (sl < 1.0E-5F) {
                sx = 1; sy = 0; sz = 0; sl = 1;
            }
            sx /= sl; sy /= sl; sz /= sl;
            if (i > first && sx * psx + sy * psy + sz * psz < 0) {
                sx = -sx; sy = -sy; sz = -sz;
            }
            psx = sx; psy = sy; psz = sz;
            float u = (i + 0.5F) / (pointCount - 1.0F);
            float w = width * (0.18F + (float) Math.sin(Math.PI * u) * 0.82F);
            quad(out,
                    x0 + sx * w, y0 + sy * w, z0 + sz * w,
                    x1 + sx * w, y1 + sy * w, z1 + sz * w,
                    x1 - sx * w, y1 - sy * w, z1 - sz * w,
                    x0 - sx * w, y0 - sy * w, z0 - sz * w,
                    color, alpha);
        }
    ...
    private static int mixColor(int a, int b, float t) {
        ...
        int r = (int) (((a >> 16) & 255) * (1.0F - t) + ((b >> 16) & 255) * t);
        int g = (int) (((a >> 8) & 255) * (1.0F - t) + ((b >> 8) & 255) * t);
        int bl = (int) ((a & 255) * (1.0F - t) + (b & 255) * t);
        return (r << 16) | (g << 8) | bl;
    }
    ...
    private static void quad(BufferBuilder out,
                             float x0, float y0, float z0,
                             float x1, float y1, float z1,
                             float x2, float y2, float z2,
                             float x3, float y3, float z3,
                             int color, float alpha) {
        tri(out, x0, y0, z0, x1, y1, z1, x2, y2, z2, color, alpha);
        tri(out, x0, y0, z0, x2, y2, z2, x3, y3, z3, color, alpha);
    }

    private static void tri(BufferBuilder out,
                            float x0, float y0, float z0,
                            float x1, float y1, float z1,
                            float x2, float y2, float z2,
                            int color, float alpha) {
        vertex(out, x0, y0, z0, color, alpha);
        vertex(out, x1, y1, z1, color, alpha);
        vertex(out, x2, y2, z2, color, alpha);
    }

    private static void colorTri(BufferBuilder out,
                                 float x0, float y0, float z0,
                                 float x1, float y1, float z1,
                                 float x2, float y2, float z2,
                                 int c0, int c1, int c2, float alpha) {
        vertex(out, x0, y0, z0, c0, alpha);
        vertex(out, x1, y1, z1, c1, 0.0F);
        vertex(out, x2, y2, z2, c2, 0.0F);
    }

    private static void vertex(BufferBuilder out, float x, float y, float z, int color, float alpha) {
        int r = (color >> 16) & 255;
        int g = (color >> 8) & 255;
        int b = color & 255;
        int a = (int) (MathHelper.clamp(alpha, 0.0F, 1.0F) * 255.0F);
        out.pos(x, y, z).color(r, g, b, a).endVertex();
    }
}
