...
/** Ice Epiphyllum, ported from the 1.12.2 Honkai blade. */
public class IceEpiphyllum extends BladeDefineBase {
    public IceEpiphyllum(BootstapContext<SlashBladeDefinition> context) {
        String key = getKey();
        context.register(createBladeKey(key), new SlashBladeDefinition(
                getBaseBladeId(BaseBladeType.HONKAI),
                ResourceUtil.getLocation(key),
                RenderDefinition.Builder.newInstance()
                        .modelName(ResourceUtil.getLocation("model/honkai/ice_epiphyllum/mdl_ice_epiphyllum.obj"))
                        .textureName(ResourceUtil.getLocation("model/honkai/ice_epiphyllum/tex_ice_epiphyllum.png"))
                        .effectColor(0x00FFFF)
                        .standbyRenderType(CarryType.KATANA)
                        .build(),
                PropertiesDefinition.Builder.newInstance()
                        .baseAttackModifier(22.0F)
                        .maxDamage(1000)
                        .defaultSwordType(List.of(SwordType.BEWITCHED))
                        .slashArtsType(ModSlashArts.SNOW_DANCE.getId())
