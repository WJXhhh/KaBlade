...
public final class ThunderboltCallArts extends SlashArts {
    ...
    public ResourceLocation doArts(ArtsType type, LivingEntity user) {
        ...
        if (ThunderboltCallEntity.spawn(level, user, target, targetAnchor,
    ...
    private static LivingEntity resolveTarget(ServerLevel level, LivingEntity user, ItemStack blade) {
        ...
        Vec3 look = normalizedLook(user);
        Vec3 end = eye.add(look.scale(TARGET_RANGE));
        AABB rayArea = user.getBoundingBox().expandTowards(look.scale(TARGET_RANGE)).inflate(1.25D);
        return level.getEntitiesOfClass(LivingEntity.class, rayArea,
                        target -> validTarget(user, target)
                                && target.getBoundingBox().inflate(target.getPickRadius() + 0.35D)
