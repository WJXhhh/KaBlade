...
/** 1.12.2 可用的 GLSL 1.20 冰霜辉光；外部光影占用 program 时自动仅用几何回退。 */
final class UtpalaAuraShader {
    private static final ResourceLocation VERTEX =
            new ResourceLocation(Main.MODID, "shaders/utpala_aura_120.vsh");
    private static final ResourceLocation FRAGMENT =
            new ResourceLocation(Main.MODID, "shaders/utpala_aura_120.fsh");
    private static int program = -1;
    private static int timeUniform = -1;
    private static int strengthUniform = -1;

    private UtpalaAuraShader() {}
    ...
    private static int createProgram() {
        ...
        try {
            ...
            strengthUniform = GL20.glGetUniformLocation(linked, "uStrength");
            return linked;
        } catch (Throwable error) {
            if (linked != 0) GL20.glDeleteProgram(linked);
            if (Main.logger != null) Main.logger.warn(
                    "Utpala Aura GLSL enhancement unavailable; using vertex-gradient fallback", error);
            return 0;
        } finally {
