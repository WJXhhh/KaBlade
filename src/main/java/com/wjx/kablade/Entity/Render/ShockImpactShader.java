...
/** 可选 GLSL 1.20 辉光增强；与外部光影冲突时保持原 program 并走几何回退。 */
final class ShockImpactShader {
    private static final ResourceLocation VERTEX =
            new ResourceLocation(Main.MODID, "shaders/shock_impact_120.vsh");
    private static final ResourceLocation FRAGMENT =
            new ResourceLocation(Main.MODID, "shaders/shock_impact_120.fsh");
    private static int program = -1;
    private static int timeUniform = -1;
    private static int strengthUniform = -1;

    private ShockImpactShader() {}
    ...
    private static int createProgram() {
        ...
        try {
            ...
            if (Main.logger != null) Main.logger.warn(
                    "Shock Impact GLSL enhancement unavailable; using fixed-pipeline fallback", error);
            return 0;
        } finally {
