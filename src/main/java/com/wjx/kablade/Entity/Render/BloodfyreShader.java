...
/** 1.12.2 / GLSL 1.20 的血焰解析着色器；光影占用 program 时自动使用顶点色回退。 */
final class BloodfyreShader {
    private static final ResourceLocation VERTEX =
            new ResourceLocation(Main.MODID, "shaders/bloodfyre_frenzy_120.vsh");
    private static final ResourceLocation FRAGMENT =
            new ResourceLocation(Main.MODID, "shaders/bloodfyre_frenzy_120.fsh");
    private static int program = -1;
    private static int timeUniform = -1;

    private BloodfyreShader() {}

    static int bind(float age) {
        if (!OpenGlHelper.shadersSupported) return 0;
        int previous = GL11.glGetInteger(GL20.GL_CURRENT_PROGRAM);
        // 不覆盖 OptiFine/光影包已经绑定的 program。
    ...
    private static int createProgram() {
        ...
        try {
            ...
            if (Main.logger != null) Main.logger.warn(
                    "Bloodfyre GLSL enhancement unavailable; using vertex-gradient fallback", error);
            return 0;
        } finally {
