...
public final class LimpidityPostPipeline {
    ...
    static boolean enqueue(EntityConceptualField entity,float partial){
        ...
        if(GL11.glGetInteger(GL20.GL_CURRENT_PROGRAM)!=0)return false;
    ...
    @SubscribeEvent public static void render(RenderWorldLastEvent event){
        ...
        if(mc.world==null||mc.getRenderViewEntity()==null)return;GlState state=GlState.capture();
        try{Framebuffer main=mc.getFramebuffer();TARGET.ensure(mc.displayWidth,mc.displayHeight);TARGET.color(main);drawAll(work,false,event.getPartialTicks());TARGET.mask(main);drawAll(work,true,event.getPartialTicks());TARGET.composite(main);}
        catch(Throwable error){disabledForSession=true;if(Main.logger!=null)Main.logger.warn("Limpidity post pipeline disabled; using direct renderer",error);try{mc.getFramebuffer().bindFramebuffer(true);drawFallback(work,event.getPartialTicks());}catch(Throwable ignored){}}
        finally{state.restore();}
    }

    @SubscribeEvent public static void reload(TextureStitchEvent.Pre event){TARGET.release();disabledForSession=false;}
    ...
    private static final class Target{
        ...
        void color(Framebuffer main){bindWithDepth(color,main.depthBuffer);clear();prepareWorld();}
        void mask(Framebuffer main){bindWithDepth(mask,main.depthBuffer);clear();prepareWorld();}
        void bindWithDepth(int texture,int depth){OpenGlHelper.glBindFramebuffer(OpenGlHelper.GL_FRAMEBUFFER,fbo);OpenGlHelper.glFramebufferTexture2D(OpenGlHelper.GL_FRAMEBUFFER,OpenGlHelper.GL_COLOR_ATTACHMENT0,GL11.GL_TEXTURE_2D,texture,0);OpenGlHelper.glFramebufferRenderbuffer(OpenGlHelper.GL_FRAMEBUFFER,OpenGlHelper.GL_DEPTH_ATTACHMENT,OpenGlHelper.GL_RENDERBUFFER,depth);GL11.glDrawBuffer(OpenGlHelper.GL_COLOR_ATTACHMENT0);check();GL11.glViewport(0,0,w,h);}
        void clear(){GL11.glColorMask(true,true,true,true);GL11.glClearColor(0,0,0,0);GL11.glClear(GL11.GL_COLOR_BUFFER_BIT);}
        ...
        void composite(Framebuffer main){int bloom=mask;for(int i=0;i<5;i++){postTarget(blurA);fullScreen(blurProgram,bloom,0,true);postTarget(blurB);fullScreen(blurProgram,blurA,0,false);bloom=blurB;}
            main.bindFramebuffer(true);GL11.glBindTexture(GL11.GL_TEXTURE_2D,scene);GL11.glCopyTexSubImage2D(GL11.GL_TEXTURE_2D,0,0,0,0,0,w,h);main.bindFramebuffer(true);fullScreen(compositeProgram,scene,color,bloom);}
        void postTarget(int texture){OpenGlHelper.glBindFramebuffer(OpenGlHelper.GL_FRAMEBUFFER,postFbo);OpenGlHelper.glFramebufferTexture2D(OpenGlHelper.GL_FRAMEBUFFER,OpenGlHelper.GL_COLOR_ATTACHMENT0,GL11.GL_TEXTURE_2D,texture,0);OpenGlHelper.glFramebufferRenderbuffer(OpenGlHelper.GL_FRAMEBUFFER,OpenGlHelper.GL_DEPTH_ATTACHMENT,OpenGlHelper.GL_RENDERBUFFER,0);GL11.glDrawBuffer(OpenGlHelper.GL_COLOR_ATTACHMENT0);check();GL11.glViewport(0,0,w,h);GL11.glClearColor(0,0,0,0);GL11.glClear(GL11.GL_COLOR_BUFFER_BIT);}
        void fullScreen(int program,int tex0,int tex1,boolean horizontal){GL11.glDisable(GL11.GL_DEPTH_TEST);GL11.glDepthMask(false);GL11.glDisable(GL11.GL_BLEND);GL20.glUseProgram(program);bindTex(program,"Source",0,tex0);if(tex1!=0)bindTex(program,"Effect",1,tex1);int bloom=GL20.glGetUniformLocation(program,"Bloom");if(bloom>=0){GL13.glActiveTexture(GL13.GL_TEXTURE2);GL11.glBindTexture(GL11.GL_TEXTURE_2D,horizontal?0:blurB);GL20.glUniform1i(bloom,2);}int size=GL20.glGetUniformLocation(program,"TexelSize");if(size>=0)GL20.glUniform2f(size,1F/w,1F/h);int dir=GL20.glGetUniformLocation(program,"Direction");if(dir>=0)GL20.glUniform2f(dir,horizontal?1:0,horizontal?0:1);quad();GL20.glUseProgram(0);GL13.glActiveTexture(GL13.GL_TEXTURE0);}
        void fullScreen(int program,int sceneTex,int effectTex,int bloomTex){GL11.glDisable(GL11.GL_DEPTH_TEST);GL11.glDepthMask(false);GL11.glDisable(GL11.GL_BLEND);GL20.glUseProgram(program);bindTex(program,"Scene",0,sceneTex);bindTex(program,"Effect",1,effectTex);bindTex(program,"Bloom",2,bloomTex);int s=GL20.glGetUniformLocation(program,"TexelSize");if(s>=0)GL20.glUniform2f(s,1F/w,1F/h);quad();GL20.glUseProgram(0);GL13.glActiveTexture(GL13.GL_TEXTURE0);}
    ...
    private static final class GlState{final int fbo,program,active,blendSrcRgb,blendDstRgb,blendSrcAlpha,blendDstAlpha,depthFunc;final IntBuffer viewport=BufferUtils.createIntBuffer(16);final int[] textures=new int[3];final boolean depth,blend,cull,lighting,texture;final boolean depthMask;GlState(){fbo=GL11.glGetInteger(GL30.GL_FRAMEBUFFER_BINDING);program=GL11.glGetInteger(GL20.GL_CURRENT_PROGRAM);active=GL11.glGetInteger(GL13.GL_ACTIVE_TEXTURE);blendSrcRgb=GL11.glGetInteger(GL14.GL_BLEND_SRC_RGB);blendDstRgb=GL11.glGetInteger(GL14.GL_BLEND_DST_RGB);blendSrcAlpha=GL11.glGetInteger(GL14.GL_BLEND_SRC_ALPHA);blendDstAlpha=GL11.glGetInteger(GL14.GL_BLEND_DST_ALPHA);depthFunc=GL11.glGetInteger(GL11.GL_DEPTH_FUNC);for(int i=0;i<3;i++){GL13.glActiveTexture(GL13.GL_TEXTURE0+i);textures[i]=GL11.glGetInteger(GL11.GL_TEXTURE_BINDING_2D);}GL13.glActiveTexture(active);GL11.glGetInteger(GL11.GL_VIEWPORT,viewport);depth=GL11.glIsEnabled(GL11.GL_DEPTH_TEST);blend=GL11.glIsEnabled(GL11.GL_BLEND);cull=GL11.glIsEnabled(GL11.GL_CULL_FACE);lighting=GL11.glIsEnabled(GL11.GL_LIGHTING);texture=GL11.glIsEnabled(GL11.GL_TEXTURE_2D);depthMask=GL11.glGetBoolean(GL11.GL_DEPTH_WRITEMASK);}static GlState capture(){return new GlState();}void restore(){OpenGlHelper.glBindFramebuffer(OpenGlHelper.GL_FRAMEBUFFER,fbo);GL20.glUseProgram(program);for(int i=0;i<3;i++){GL13.glActiveTexture(GL13.GL_TEXTURE0+i);GL11.glBindTexture(GL11.GL_TEXTURE_2D,textures[i]);}GL13.glActiveTexture(active);GL11.glViewport(viewport.get(0),viewport.get(1),viewport.get(2),viewport.get(3));GL14.glBlendFuncSeparate(blendSrcRgb,blendDstRgb,blendSrcAlpha,blendDstAlpha);GL11.glDepthFunc(depthFunc);set(GL11.GL_DEPTH_TEST,depth);set(GL11.GL_BLEND,blend);set(GL11.GL_CULL_FACE,cull);set(GL11.GL_LIGHTING,lighting);set(GL11.GL_TEXTURE_2D,texture);GL11.glDepthMask(depthMask);}static void set(int cap,boolean on){if(on)GL11.glEnable(cap);else GL11.glDisable(cap);}}
}
