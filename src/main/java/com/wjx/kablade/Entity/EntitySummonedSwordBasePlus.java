...
public class EntitySummonedSwordBasePlus extends Entity implements IThrowableEntity {
    ...
    public EntitySummonedSwordBasePlus(World par1World)
    {
        super(par1World);
    ...
    public EntitySummonedSwordBasePlus(World par1World, EntityLivingBase entityLiving, float AttackLevel, float roll){
        this(par1World,entityLiving,AttackLevel);
        this.setRoll(roll);
    }

    public EntitySummonedSwordBasePlus(World par1World, EntityLivingBase entityLiving, float AttackLevel)
    {
        this(par1World);
    ...
    private static final DataParameter<Integer> TARGET_ENTITY_ID = EntityDataManager.createKey(EntitySummonedSwordBasePlus.class, DataSerializers.VARINT);
    private static final DataParameter<Integer> INTERVAL = EntityDataManager.createKey(EntitySummonedSwordBasePlus.class, DataSerializers.VARINT);
    private static final DataParameter<Integer> COLOR = EntityDataManager.createKey(EntitySummonedSwordBasePlus.class, DataSerializers.VARINT);
public static final DataParameter<Integer> BRIGHT = EntityDataManager.createKey(EntitySummonedSwordBasePlus.class,DataSerializers.VARINT);
public static final DataParameter<Float> BRIGHTNESS = EntityDataManager.createKey(EntitySummonedSwordBasePlus.class,DataSerializers.FLOAT);
    ...
    public boolean doTargeting(){
        ...
        // 目标由 SA 入口解析并传入，避免每把剑在多个 Tick 内重复扫描 30 格范围。
    ...
    protected RayTraceResult getRayTraceResult(){
        ...
        for(Predicate<Entity> selector : selectors){
            ...
            for (i = 0; i < list.size(); ++i)
            {
                ...
                if(entity1 instanceof EntitySummonedSwordBasePlus)
