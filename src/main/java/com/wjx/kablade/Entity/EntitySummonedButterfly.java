...
public class EntitySummonedButterfly extends Entity implements IThrowableEntity {
    ...
    public EntitySummonedButterfly(World par1World)
    {
        super(par1World);
    ...
    public EntitySummonedButterfly(World par1World, EntityLivingBase entityLiving, float AttackLevel, float roll){
        this(par1World,entityLiving,AttackLevel);
        this.setRoll(roll);
    }

    public EntitySummonedButterfly(World par1World, EntityLivingBase entityLiving, float AttackLevel)
    {
        this(par1World);
    ...
    public EntitySummonedButterfly(World par1World, EntityLivingBase entityLiving, float AttackLevel, double x, double y , double z, float rotationPitch, float rotationYaw){
        this(par1World, entityLiving, AttackLevel);
    ...
    private static final DataParameter<Integer> THROWER_ENTITY_ID = EntityDataManager.createKey(EntitySummonedButterfly.class, DataSerializers.VARINT);
    private static final DataParameter<Integer> LIFETIME = EntityDataManager.createKey(EntitySummonedButterfly.class, DataSerializers.VARINT);
    private static final DataParameter<Float> ROLL = EntityDataManager.createKey(EntitySummonedButterfly.class, DataSerializers.FLOAT);
    private static final DataParameter<Integer> TARGET_ENTITY_ID = EntityDataManager.createKey(EntitySummonedButterfly.class, DataSerializers.VARINT);
    private static final DataParameter<Integer> INTERVAL = EntityDataManager.createKey(EntitySummonedButterfly.class, DataSerializers.VARINT);
    private static final DataParameter<Integer> COLOR = EntityDataManager.createKey(EntitySummonedButterfly.class, DataSerializers.VARINT);
public static final DataParameter<Integer> BRIGHT = EntityDataManager.createKey(EntitySummonedButterfly.class,DataSerializers.VARINT);
public static final DataParameter<Float> BRIGHTNESS = EntityDataManager.createKey(EntitySummonedButterfly.class,DataSerializers.FLOAT);
    ...
    protected void entityInit() {
        ...
        this.dataManager.register(BRIGHT,-1);

        this.dataManager.register(BRIGHTNESS,-1f);
    }
    ...
    public Entity getRayTrace(Entity owner, double reachMax, float expandFactor, float expandBorder) {
        ...
        for (Entity entity : list) {
            ...
            AxisAlignedBB axisalignedbb = entity.getEntityBoundingBox().grow(borderSize, borderSize, borderSize);
                ...
                if(!world.isRemote){
                    ...
                    if(!blade.isEmpty() && ridingEntity instanceof EntityLivingBase){
                        if(thrower != null){
                            ...
                            blade.getItem().hitEntity(blade,(EntityLivingBase)ridingEntity,(EntityLivingBase)thrower);
                        }
    ...
    protected RayTraceResult getRayTraceResult(){
        ...
        for(Predicate<Entity> selector : selectors){
            ...
            for (i = 0; i < list.size(); ++i)
            {
                ...
                if(entity1 instanceof EntitySummonedButterfly)
                    if(((EntitySummonedButterfly) entity1).getThrower() == this.getThrower())
                        continue;
    ...
    {
        if (this.getDataManager().get(BRIGHT) <= -1){
            float f1 = 0.5F;
            ...
            return j | k << 16;
        }else return this.dataManager.get(BRIGHT);
    ...
    {
        if (this.dataManager.get(BRIGHTNESS) <= -1) {
        ...
        else return this.dataManager.get(BRIGHTNESS);
    }
